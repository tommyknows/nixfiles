{
  ...
  programs = {
    ...
    git = {
      enable = true;
      lfs.enable = true;
      # Sign only where a signing key is configured (the Macs set
      # signing.key to their Secretive pubkey in hosts/<host>/home.nix). The
      # server sets no key → no signing, rather than failing every commit.
      signing.signByDefault = lib.mkDefault (config.programs.git.signing.key != null);
