...
# same. See jj-migration-map.html for the broader plan.
{
  ...
  programs.jujutsu = {
    ...
    settings = {
      ...
      # SSH commit signing via the same Secretive key git uses. `signing.key` is
      # host-specific (set in hosts/<host>/home.nix for git); reuse it. Only when
      # a key is set (the Macs) — the server has none and doesn't sign.
